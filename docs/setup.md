...
The fix has two parts, and the first one alone is not enough — this was verified the hard way.

Part one is authorizing through Meta's own "Facebook Login for Business" product with a `config_id` instead of a `scope` param, which shows Meta's own asset picker during authorization so the person explicitly selects which Page(s) to share. This is necessary but **not sufficient**: even after picking the Page in that picker, with `pages_show_list` confirmed `"granted"` via `GET /me/permissions`, `GET /me/accounts` still returned an empty list for it. No error, no delay that fixes itself — just permanently empty, specifically for this edge, for this class of Page.

Part two, the piece that actually makes it work: querying the Page directly by ID (`GET /{page-id}?fields=id,name,access_token`) with that same user token returns a real Page access token immediately. The access was real the whole time; `/me/accounts` just doesn't enumerate Business-Portfolio-assigned Pages reliably. The missing piece is knowing *which* Page ID to query without asking the person to type it in — that comes from `GET /debug_token?input_token={token}&access_token={app_id}|{app_secret}`, whose response includes `granular_scopes`, Meta's own record of exactly which asset IDs a config_id-based authorization covers, independent of what `/me/accounts` lists. `getFacebookPages()` in `lib/meta/oauth.ts` reads the Page IDs out of `granular_scopes` and resolves each one via the direct Page-node query — it only falls back to `/me/accounts` when a token has no `granular_scopes` at all (classic scope-based Facebook Login, where this Business-Portfolio quirk doesn't apply).
...
4. Token de acceso: "Token de acceso de usuario" — not "Token de acceso de usuario del sistema". The system-user variant needs a Business Portfolio login of its own and a different code path (system user tokens aren't obtained through this same authorization-code exchange); it also would not have fixed the `/me/accounts` gap by itself — that gap turned out to be about the API edge, not the token type, see above.
